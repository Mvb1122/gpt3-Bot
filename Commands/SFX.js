...
const { MakeSFX } = require('../VoiceV2');
const fp = require('fs/promises')

module.exports = {
    data: new SlashCommandBuilder()
        .setName('sfx')
        .setDescription("Generates sound effects!")
    ...
    async execute(interaction) {
        ...
        MakeSFX(prompt, `./Temp/${interaction.user.id}_music.wav`, length).then((output) => {
            time_taken = ((performance.now() - time_taken) / 1000).toFixed(2);
            interaction.editReply({
                content: "Here's your sound effect! Prompt:```" + prompt + "```\nTime Taken: " + time_taken + "s",
                files: [output]
            }).then(() => {
                fp.unlink(output);
            })
