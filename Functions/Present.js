...
module.exports = {
  ...
  json:
  {
    ...
    "parameters": {
      ...
      "properties": {
        ...
        "slidelimit": {
          "type": "number",
          "description": "The number of slides to use."
        }
      }
    },
    "required": ["text"]
   ...
   * @param {{topic: string, showmessages: boolean, voiceover: boolean, research: boolean, slidelimit: number}} parameters Parameters from AI.
  ...
  execute(parameters, DiscordMessage = null, messages) {
    ...
    if (!DiscordMessage) return "Unable to call this function right now!"
      ...
      return new Promise(async mainRes => {
        ...
        let slidelimitText = parameters.slidelimit ? " **Please use a slide count limit of " + parameters.slidelimit + "!**" : ""
        ...
        outputMessage.edit(messageContent += "\nIdeation complete!");
        ...
        internalMessages = internalMessages.concat(  
          ...
          NewMessage("User", `Now, let's make that exact presentation output for Marp on the topic of ${parameters.topic}. As a reminder, please only write the presentation. Please make your presentation as long as is necessary/limited!${slidelimitText} Make sure to have the number of slides requested. Do not write anything except for the presentation. Do not write a slide showing your sources. Do not include \`\`\` in your answer unless you're creating a code block. Make sure to split your presentation into pages using ---\nAdditionally, make sure to use markdown syntax for images, like ![](URL HERE)\nThe images are in the previous message.\n\nMAKE SURE to split your slides with ---\nPlease use your earlier reasoning on the topic of ${parameters.topic} and make the final presentation!`)
        );
            ...
            marpProcess.on('close', async (code) => {
              if (code !== 0) {
                ...
                */
                ...
                args = args.concat([
                  "-filter_complex", `"${filterMap.trim()} concat=n=${files.length}:v=1:a=1 [v] [a]"`, "-map", "[v]", "-map", "[a]", "-vcodec", "libx265", "-crf", "28", "-preset", "ultrafast", `"${FinalVideoOutput}"` 
                  // Old options "-r", "2", "-pix_fmt", "yuv420p", "-c:v", "h264_nvenc", "-cq", "40"
                ]);
                ...
                try {
                  await (await outputMessage).edit({
                    content: "Finished video!" + TimeTakenMessage,
                    files: [FinalVideoOutput, VoiceOversFile]
                  });
                } catch {
                  outputMessage.edit({
                    content: "The finished video ended up being too big! Just ask Micah for it.",
                    files: [VoiceOversFile]
                  })
                }
        ...
        marpProcess.on('close', async (code) => {
          if (code !== 0) {
            ...
            // If we're not doing a voice over, add a button 
            let row;
            if (!parameters.voiceover) {
              const acceptButton = new Discord.ButtonBuilder()
                .setEmoji('🔊')
                .setCustomId("makevoiceoverbutton")
                .setLabel("Make a voiceover!")
            ...
            if (!parameters.voiceover) {
              outputMessage.awaitMessageComponent().then(async (resp) => {
                // Remove the button from the message and make a new message.
                outputMessage.edit({components: []});
                outputMessage = await resp.reply("Starting video generation!");
                parameters.voiceover = true;
                await DoVoiceOver();
              });
            } else await DoVoiceOver();
